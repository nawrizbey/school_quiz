...
export default function CreateQuiz() {
  ...
  useEffect(() => {
    ...
    const teacherSubject = localStorage.getItem('teacherSubject') as Subject | '';
    const teacherFullName = localStorage.getItem('teacherFullName') || '';
    const teacherRole = localStorage.getItem('teacherRole');

    // Set author name
    setAuthorName(teacherFullName);

    // If teacher (not super admin), set subject and day automatically
    if (teacherRole !== 'super_admin' && teacherSubject) {
      setSubject(teacherSubject);
      const subjectInfo = SUBJECTS.find(s => s.value === teacherSubject);
      if (subjectInfo) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!teacherId) {
        alert(translations.errors.unauthorized);
        setLoading(false);
        return;
      }
      ...
      alert(translations.createQuiz.testCreated);
      ...
      alert(translations.createQuiz.errorCreating);
    } finally {
      setLoading(false);
    }
  };

  const teacherRole = typeof window !== 'undefined' ? localStorage.getItem('teacherRole') : null;
  const teacherSubject = typeof window !== 'undefined' ? localStorage.getItem('teacherSubject') : '';
      ...
      <div className="max-w-4xl mx-auto">
        <div className="bg-white rounded-2xl shadow-lg p-6 md:p-8">
          <div className="flex justify-between items-center mb-6">
            <h1 className="text-3xl font-bold text-gray-800">{translations.createQuiz.title}</h1>
            <button
              onClick={() => router.back()}
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                {teacherRole === 'super_admin' ? (
                  ...
                  <div className="px-4 py-3 bg-gray-100 border border-gray-300 rounded-lg text-gray-800">
                    {subjectLabel} ({dayLabel})
                  </div>
                )}
              </div>
            ...
            <div className="flex gap-4">
              ...
              >
                {loading ? translations.createQuiz.creating : translations.createQuiz.createTest}
              </button>
              <button
                type="button"
                onClick={() => router.back()}
